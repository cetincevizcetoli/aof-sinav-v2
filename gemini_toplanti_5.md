...
## Sorun Başlığı
Çoklu Cihaz Senkronizasyonu: “İlerlemeyi Sıfırla” işlemi bir cihazda yapıldığında, diğer cihaz bazen eski veriyi geri yazıyor; reset_at filtresine rağmen bazı push/pull dizilerinde eski ilerleme baskın çıkıyor.

## Gözlemler
- Cihaz A’da sıfırlandıktan sonra Cihaz B F5/CTRL+F5 ile yenilese bile 4/21 ilerleme kalabiliyor.
- Cihaz A yeniden yüklenince Cihaz B’nin 4/21 verisini geri çekebiliyor.
- Sunucuda reset_at uygulanıyor; client tarafı server_reset_at/last_reset_ack mantığı ile push skip ediyor.

## Şüphe Noktaları
- Eski kayıtların updated_at/started_at alanları 0 veya eksik geldiğinde sunucu tarafında zaman damgası sonradan `time()` ile doldurulunca karşılaştırma anomalisi oluşabilir.
- Client pull → local reset sonra bazı edge durumlarda eski local queue (sync_queue) push edebiliyor mu?
- Visibility ve ağ kesintisi altında sırayla: A(wipe)→B(pull fail/skip)→B(push later) gibi yarış durumları.

## Yardım Talebi
Reset işleminden sonra sunucu/istemci tarafında zamana dayalı tutarlılığı garantilemek için önerilen canonical desen nedir? Özellikle timestamp alanı eksik/0 ise güvenli reddetme ve client queue drain sırası için best‑practice istiyoruz.
Tarih: 2025-11-29T21:14:33
...
```javascript
const STATIC_CACHE = 'aof-asistan-v2-static-v1.1.34';
const DATA_CACHE = 'aof-asistan-v2-data-v1.1.34';
const TTL_MS = 300000;
const STATIC_ASSETS = [
    './',
    './index.html',
...
export class SyncManager {
    ...
    async pushAll(){ const token = this.getToken(); if(!token) return false; const serverReset = await this.db.getProfile('server_reset_at'); const ack = await this.db.getProfile('last_reset_ack'); if (serverReset && (!ack || ack < serverReset)) return true; const progress = await this.db.getAllProgress(); const stats = await this.db.getUserStats(); const history = await this.db.getHistory(); const sessions = await (this.db.getAllSessions ? this.db.getAllSessions() : Promise.resolve([])); const payload = { progress: progress.map(p=>({ ...p, updated_at: p.updated_at||0 })), stats: { ...stats, updated_at: stats.updated_at||0 }, history, sessions }; const r = await fetch(`${this.base}/sync.php?action=push&token=${encodeURIComponent(token)}`,{ method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(payload) }); if(r.ok){ const ts = Date.now(); await this.db.setProfile('last_sync', ts); const email = await this.db.getProfile('account_email'); const accounts = (await this.db.getProfile('accounts')) || []; const i = Array.isArray(accounts) ? accounts.findIndex(a => a && a.email === email) : -1; if (i >= 0) { accounts[i].lastSync = ts; await this.db.setProfile('accounts', accounts); } } return r.ok; }
