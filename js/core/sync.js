export class SyncManager {
    ...
    async pullAll(){ const token = this.getToken(); if(!token) return false; const r = await fetch(`${this.base}/sync.php?action=pull&token=${encodeURIComponent(token)}`,{ headers:{ 'Authorization': `Bearer ${token}` } }); if(!r.ok) return false; const j = await r.json(); const p = (j.data && j.data.progress) || []; const s = (j.data && j.data.stats) || { xp:0, streak:0, totalQuestions:0, updated_at:0 }; const h = (j.data && j.data.history) || []; const ss = (j.data && j.data.sessions) || []; const serverEmpty = (p.length===0) && (h.length===0) && (ss.length===0); if (serverEmpty && this.db.resetProgressOnly) { await this.db.resetProgressOnly(); } else { for(const item of p){ await this.db.saveProgress(item.id,{ id:item.id, level:item.level, nextReview:item.nextReview, correct:item.correct, wrong:item.wrong, updated_at:item.updated_at||Date.now() }); } await this.db.updateUserStats(s); for(const hi of h){ await this.db.logActivity(hi.lesson, hi.unit, !!hi.isCorrect, hi.qid, hi.given_option, hi.cycle_no); } if (ss && ss.length>0 && this.db.importSessions) { await this.db.importSessions(ss); } } const ts = Date.now(); await this.db.setProfile('last_sync', ts); const email = await this.db.getProfile('account_email'); const accounts = (await this.db.getProfile('accounts')) || []; const i = Array.isArray(accounts) ? accounts.findIndex(a => a && a.email === email) : -1; if (i >= 0) { accounts[i].lastSync = ts; await this.db.setProfile('accounts', accounts); } try { document.dispatchEvent(new CustomEvent('app:data-updated')); } catch(e){} return true; }
