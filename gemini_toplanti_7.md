...
- .
  ...
  - gemini_toplanti_4.md
  - gemini_toplanti_5.md
  - gemini_toplanti_6.md
...
```javascript
const STATIC_CACHE = 'aof-asistan-v2-static-v1.1.38';
const DATA_CACHE = 'aof-asistan-v2-data-v1.1.38';
const TTL_MS = 300000;
const STATIC_ASSETS = [
    './',
    './index.html',
...
try { $pdo->exec('CREATE INDEX idx_ss_user_started ON study_sessions(user_id, started_at)'); } catch (Throwable $e) {}
...
$logFile = __DIR__.'/../private/debug_sync.log';
...
if ($a === 'push') {
    ...
    foreach ($progress as $p) {
        ...
        if ($resetAt && ($inc === 0 || $inc < $resetAt)) { try { $msg = "Zombie rejected (progress): uid=$user id=$pid inc=$inc reset=$resetAt"; @file_put_contents($logFile, '['.date('Y-m-d H:i:s').'] '.$msg."\n", FILE_APPEND); } catch (Throwable $e) {} continue; }
    ...
    if (isset($in['stats'])) {
        ...
        if ($resetAt && ($incS === 0 || $incS < $resetAt)) { try { $msg = "Zombie rejected (stats): uid=$user inc=$incS reset=$resetAt"; @file_put_contents($logFile, '['.date('Y-m-d H:i:s').'] '.$msg."\n", FILE_APPEND); } catch (Throwable $e) {} } else {
    ...
    if (isset($in['history']) && is_array($in['history'])) {
        ...
        foreach ($in['history'] as $h) { $hd = intval($h['date']??0); if ($resetAt && ($hd === 0 || $hd < $resetAt)) { try { $msg = "Zombie rejected (history): uid=$user date=$hd reset=$resetAt uuid=".($h['uuid']??''); @file_put_contents($logFile, '['.date('Y-m-d H:i:s').'] '.$msg."\n", FILE_APPEND); } catch (Throwable $e) {} continue; } $ins->execute([$user, ($hd ?: $nowMs), $h['lesson']??'', intval($h['unit']??0), intval(($h['isCorrect']??0)?1:0), (string)($h['uuid']??''), (string)($h['qid']??''), (string)($h['given_option']??''), intval($h['cycle_no']??0)]); }
    }
    if (isset($in['sessions']) && is_array($in['sessions'])) {
        ...
        foreach ($in['sessions'] as $s) { $st = intval($s['started_at']??0); if ($resetAt && ($st === 0 || $st < $resetAt)) { try { $msg = "Zombie rejected (session): uid=$user start=$st reset=$resetAt uuid=".($s['uuid']??''); @file_put_contents($logFile, '['.date('Y-m-d H:i:s').'] '.$msg."\n", FILE_APPEND); } catch (Throwable $e) {} continue; } $insS->execute([$user, $s['lesson']??'', intval($s['unit']??0), $s['mode']??'study', ($st ?: $nowMs), intval($s['ended_at']??0), (string)($s['uuid']??''), intval($s['cycle_no']??0)]); }
    }
    ok(['pushed'=>true]);
} elseif ($a === 'pull') {
...
export class ExamDatabase {
    ...
    async wipeDevice(){
        try { if (this.db) { try { this.db.close(); } catch{} } } catch{}
        return new Promise((resolve)=>{
            try {
                const req = indexedDB.deleteDatabase(this.dbName);
                req.onsuccess = () => { try { localStorage.clear(); } catch{} resolve(true); };
                req.onerror = () => { try { localStorage.clear(); } catch{} resolve(false); };
            } catch(e){ try { localStorage.clear(); } catch{} resolve(false); }
        });
    }
...
export class SyncManager {
    ...
    async pullAll(){ const token = this.getToken(); if(!token) return false; const r = await fetch(`${this.base}/sync.php?action=pull&token=${encodeURIComponent(token)}`,{ headers:{ 'Authorization': `Bearer ${token}` } }); if(!r.ok) return false; const j = await r.json(); const p = (j.data && j.data.progress) || []; const s = (j.data && j.data.stats) || { xp:0, streak:0, totalQuestions:0, updated_at:0 }; const h = (j.data && j.data.history) || []; const ss = (j.data && j.data.sessions) || []; const resetAt = (j.data && j.data.reset_at) || 0; if (resetAt) await this.db.setProfile('server_reset_at', resetAt); const serverEmpty = (p.length===0) && (h.length===0) && (ss.length===0); if (serverEmpty && this.db.resetProgressOnly) { await this.db.resetProgressOnly(); if (resetAt) await this.db.setProfile('last_reset_ack', resetAt); console.warn('🚨 Sunucu sıfırlaması algılandı! Yerel veri imha ediliyor...'); try { alert('Verileriniz başka bir cihazda sıfırlandığı için senkronize edildi.'); } catch(e){} } else { for(const item of p){ await this.db.saveProgress(item.id,{ id:item.id, level:item.level, nextReview:item.nextReview, correct:item.correct, wrong:item.wrong, updated_at:item.updated_at||0 }); } await this.db.updateUserStats(s); for(const hi of h){ await this.db.logActivity(hi.lesson, hi.unit, !!hi.isCorrect, hi.qid, hi.given_option, hi.cycle_no); } if (ss && ss.length>0 && this.db.importSessions) { await this.db.importSessions(ss); } } const ts = Date.now(); await this.db.setProfile('last_sync', ts); const email = await this.db.getProfile('account_email'); const accounts = (await this.db.getProfile('accounts')) || []; const i = Array.isArray(accounts) ? accounts.findIndex(a => a && a.email === email) : -1; if (i >= 0) { accounts[i].lastSync = ts; await this.db.setProfile('accounts', accounts); } try { document.dispatchEvent(new CustomEvent('app:data-updated')); } catch(e){} return true; }
    ...
    async autoSync(){
        ...
        for(const hi of remoteH){ if (!lastTs || (hi.date||0) > lastTs) { await this.db.logActivity(hi.lesson, hi.unit, !!hi.isCorrect, hi.qid, hi.given_option, hi.cycle_no); historyAdded = true; } }
...
export class Dashboard {
    ...
    async getAccountStatusText(){
        ...
        const serverReset = await this.db.getProfile('server_reset_at');
        const ack = await this.db.getProfile('last_reset_ack');
        const warn = serverReset && (!ack || ack < serverReset) ? ' • Sunucu sıfırlaması algılandı' : '';
        return hasToken ? `Üye${accEmail?` • ${accEmail}`:''}${lastSync?` • Son Senkron: ${new Date(lastSync).toLocaleString()}`:''}${warn}` : 'Misafir (Veriler sadece bu cihazda)';
    }
    ...
    async render() {
        ...
        if (!window.SINGLE_DEVICE_MODE && !authGate.hasToken() && !localStorage.getItem('guest_mode') && !inSession) {
    ...
    openSettings() {
        ...
        (async () => {
            await this.ensureActiveAccountToken();
            const hasToken = !!localStorage.getItem('auth_token');
            const btnUpd = document.getElementById('btn-update-cred'); if (btnUpd) btnUpd.style.display = window.SINGLE_DEVICE_MODE ? 'none' : (hasToken ? 'flex' : 'none');
            const btnLogout = document.getElementById('btn-logout'); if (btnLogout) btnLogout.style.display = window.SINGLE_DEVICE_MODE ? 'none' : (hasToken ? 'flex' : 'none');
        })();

        document.getElementById('btn-sync-now').onclick = async () => {
            const sm = new SyncManager(this.db);
            const t = sm.getToken();
            if (!t) { window.openAuthSync(); return; }
            let okPush = false, okPull = false;
            try { okPush = await sm.pushAll(); } catch {}
            try { okPull = await sm.pullAll(); } catch {}
            try { document.dispatchEvent(new CustomEvent('app:data-updated')); } catch {}
            const pill = document.querySelector('.account-pill'); if (pill) { pill.textContent = await this.getAccountStatusText(); }
            const ov = document.getElementById('settings-menu-overlay'); if (ov) ov.remove();
            if (!okPush && !okPull) { try { alert('Senkronizasyon için giriş yapın'); } catch{} }
        };
        ...
        window.doLogoutConfirmed = async () => {
            ...
            await (async () => { localStorage.removeItem('auth_token'); await this.db.setProfile('account_email',''); await this.db.setProfile('accounts', []); await this.db.setProfile('server_reset_at', 0); await this.db.setProfile('last_reset_ack', 0); await this.db.setProfile('last_sync', 0); const mini=document.getElementById('account-mini'); if (mini) mini.textContent='Misafir'; await this.refreshAccountStatus(); })();
            ...
            try { if (this.db && this.db.wipeDevice) { await this.db.wipeDevice(); } } catch{}
            try { if (navigator.serviceWorker && navigator.serviceWorker.getRegistrations) { const regs = await navigator.serviceWorker.getRegistrations(); for (const r of regs) { try { await r.unregister(); } catch{} } } } catch{}
            try { if (window.caches && window.caches.keys) { const keys = await caches.keys(); await Promise.all(keys.map(k => caches.delete(k))); } } catch{}
            this.render();
        ...
        window.forceSyncNow = async () => { const sm = new SyncManager(this.db); await sm.autoSync(); const pill = document.querySelector('.account-pill'); if (pill) { pill.textContent = await this.getAccountStatusText(); } document.getElementById('settings-menu-overlay').remove(); };
        ...
        window.openAuthSync = async () => {
            ...
            window.logoutNow = async () => { localStorage.removeItem('auth_token'); await this.db.setProfile('account_email',''); await this.db.setProfile('accounts', []); await this.db.setProfile('server_reset_at', 0); await this.db.setProfile('last_reset_ack', 0); await this.db.setProfile('last_sync', 0); const mini=document.getElementById('account-mini'); if (mini) mini.textContent='Misafir'; await this.refreshAccountStatus(); const m=document.getElementById('auth-sync-modal'); if (m) m.remove(); try { if (this.db && this.db.wipeDevice) { await this.db.wipeDevice(); } } catch{} try { if (navigator.serviceWorker && navigator.serviceWorker.getRegistrations) { const regs = await navigator.serviceWorker.getRegistrations(); for (const r of regs) { try { await r.unregister(); } catch{} } } } catch{} try { if (window.caches && window.caches.keys) { const keys = await caches.keys(); await Promise.all(keys.map(k => caches.delete(k))); } } catch{} this.render(); };
        };

        window.logoutNow = async () => { localStorage.removeItem('auth_token'); await this.db.setProfile('account_email',''); await this.db.setProfile('accounts', []); await this.db.setProfile('server_reset_at', 0); await this.db.setProfile('last_reset_ack', 0); await this.db.setProfile('last_sync', 0); const mini=document.getElementById('account-mini'); if (mini) mini.textContent='Misafir'; await this.refreshAccountStatus(); const s=document.getElementById('settings-menu-overlay'); if (s) s.remove(); try { if (this.db && this.db.wipeDevice) { await this.db.wipeDevice(); } } catch{} try { if (navigator.serviceWorker && navigator.serviceWorker.getRegistrations) { const regs = await navigator.serviceWorker.getRegistrations(); for (const r of regs) { try { await r.unregister(); } catch{} } } } catch{} try { if (window.caches && window.caches.keys) { const keys = await caches.keys(); await Promise.all(keys.map(k => caches.delete(k))); } } catch{} this.render(); try { location.reload(); } catch{} };
    }
