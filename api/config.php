...
$DB_HOST = $env['AOF_MY_HOST'] ?? getenv('AOF_MY_HOST') ?? $env['DB_HOST'] ?? getenv('DB_HOST') ?? '127.0.0.1';
$DB_PORT = (int)($env['AOF_MY_PORT'] ?? getenv('AOF_MY_PORT') ?? $env['DB_PORT'] ?? getenv('DB_PORT') ?? 3306);
$DB_NAME = $env['AOF_MY_DB'] ?? getenv('AOF_MY_DB') ?? $env['DB_NAME'] ?? getenv('DB_NAME') ?? 'aof_sinav_v2';
$DB_USER = $env['AOF_MY_USER'] ?? getenv('AOF_MY_USER') ?? $env['DB_USER'] ?? getenv('DB_USER') ?? 'root';
$DB_PASS = $env['AOF_MY_PASS'] ?? getenv('AOF_MY_PASS') ?? $env['DB_PASS'] ?? getenv('DB_PASS') ?? '';
$APP_SECRET = $env['AOF_API_SECRET'] ?? getenv('AOF_API_SECRET') ?? $env['APP_SECRET'] ?? getenv('APP_SECRET') ?? 'aof-secret-please-change';
